# Claude Code Project Instructions — unicode-segmentation-kotlin

## Project Overview

This is **unicode-segmentation-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`unicode-segmentation`](https://crates.io/crates/unicode-segmentation).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/unicode-segmentation/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/unicode-segmentation>
